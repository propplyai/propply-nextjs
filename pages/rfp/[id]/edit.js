...
export default function EditRFPPage() {
  ...
  const loadRFPData = async () => {
    try {
      ...
      // Populate form with existing RFP data
      setFormData({
        property_id: rfp.property_id || '',
        compliance_report_id: rfp.compliance_report_id || '',
        project_title: rfp.project_title || '',
        project_description: rfp.project_description || '',
        project_type: rfp.project_type || 'compliance_fix',
        scope_of_work: rfp.scope_of_work || '',
        technical_requirements: rfp.technical_requirements || '',
        materials_specifications: rfp.materials_specifications || '',
        quality_standards: rfp.quality_standards || '',
        project_timeline_days: rfp.project_timeline_days || '',
        deadline_date: rfp.deadline_date || '',
        budget_range_min: rfp.budget_range_min || '',
        budget_range_max: rfp.budget_range_max || '',
        urgency_level: rfp.urgency_level || 'normal',
        compliance_issues: rfp.compliance_issues || [],
        regulatory_requirements: rfp.regulatory_requirements || '',
        permit_requirements: rfp.permit_requirements || '',
        inspection_requirements: rfp.inspection_requirements || ''
      });

      // Set compliance issues for editing
      if (rfp.compliance_issues && rfp.compliance_issues.length > 0) {
        setSelectedComplianceIssues(rfp.compliance_issues);
        setGenerateFromCompliance(true);
      }
    } catch (error) {
    ...
    <Layout user={user} onLogout={handleLogout}>
      <div className="container-modern py-8">
        ...
        <div className="flex items-center space-x-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold gradient-text">Edit RFP</h1>
            <p className="text-slate-400">Update your Request for Proposal</p>
          </div>
        </div>
