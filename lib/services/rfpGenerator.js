...
export class RFPGenerator {
  ...
  async getRFPData(rfpProjectId) {
    ...
    // If no compliance report is linked, fetch the latest one for the property
    if (!rfp.compliance_reports || rfp.compliance_reports.length === 0) {
      const { data: latestReport, error: reportError } = await this.supabase
        .from('compliance_reports')
        .select('id, report_data, compliance_score, overall_score, created_at')
        .eq('property_id', rfp.property_id)
        .order('created_at', { ascending: false })
        .limit(1)
        .single();

      if (!reportError && latestReport) {
        rfp.compliance_reports = [latestReport];
      }
    }
  ...
  async generateMainRFPDocument(rfpData) {
    ...
    const complianceData = compliance_reports?.[0]?.report_data || {};
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f8fafc;">
            ...
            <td style="padding: 40px;">
                  ...
                  <td style="padding: 20px;">
                    <table role="presentation" style="width: 100%; border-collapse: collapse;">
                      ...
                      <tr>
                        ...
                        <td style="padding: 8px 0; color: #1e293b; font-size: 14px;">${rfpData.properties?.address || 'N/A'}, ${rfpData.properties?.city || 'N/A'}, ${rfpData.properties?.state || 'N/A'}</td>
                      </tr>
                      <tr>
              ...
              <div style="margin: 20px 0; padding: 15px; background-color: #fef3c7; border-left: 4px solid #d97706; border-radius: 4px;">
                ...
                <p style="margin: 5px 0 0 0; color: #78350f; font-size: 14px;">
                  This project addresses compliance issues at a property with a compliance score of ${rfpData.properties.compliance_score}% and ${rfpData.properties.active_violations || 0} active violations.
                </p>
              </div>
              ` : ''}
  ...
  async saveRFPDocuments(rfpProjectId, documents) {
    ...
    const { data: existingDocs } = await this.supabase
      ...
      .eq('rfp_project_id', rfpProjectId);

    const existingTypes = new Set(existingDocs?.map(doc => doc.document_type) || []);

    // Delete existing documents to recreate them
    ...
    return {
      regenerated: existingDocs && existingDocs.length > 0,
