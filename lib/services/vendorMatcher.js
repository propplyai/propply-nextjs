...
export class VendorMatcher {
   ...
   * Supports both NYC and Philadelphia data structures
   */
  VIOLATION_TO_CATEGORY = {
    ...
    // Philadelphia fields
    li_violations: ['hpd'], // L&I code violations map to general contractors
    li_permits: ['dob'],    // L&I permits (may indicate structural work)
    li_investigations: ['hpd'], // Investigations typically for maintenance issues
    li_appeals: ['dob']     // Appeals often related to structural/zoning
  };
  ...
  identifyNeededCategories(violationData) {
    ...
    // PHILADELPHIA FIELDS
    // Check L&I violations (code violations)
    if (violationData.li_violations_active > 0) {
      categories.add('hpd'); // General contractors for property maintenance
    }

    // Check L&I permits (may indicate recent construction/structural work)
    if (violationData.li_permits_total > 0 || violationData.li_permits_recent > 0) {
      categories.add('dob'); // Structural engineers & building code specialists
    }

    // Check L&I investigations (inspection history)
    if (violationData.li_investigations_total > 5) {
      // Many investigations suggests ongoing issues
      categories.add('hpd'); // Property maintenance contractors
    }

    // Check L&I appeals (property disputes)
    if (violationData.li_appeals_total > 0) {
      categories.add('dob'); // May need structural/zoning experts
    }

    // Check L&I certifications (fire safety, facade, etc.)
    if (violationData.li_certifications_expired > 0) {
      categories.add('fire_safety'); // Expired fire safety certifications
    }
