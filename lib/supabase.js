...
export const createServerSupabaseClient = ({ req, res } = {}) => {
  // If req/res are provided, extract the auth token from cookies
  let authToken = null;
  if (req && req.headers.cookie) {
    ...
    const authCookie = cookies['supabase.auth.token'] || cookies['sb-access-token'] || cookies['sb-refresh-token'];
    if (authCookie) {
      try {
