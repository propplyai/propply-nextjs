...
export const createServerSupabaseClient = ({ req, res } = {}) => {
  // Extract the auth token from Authorization header (sent by client)
  let authToken = null;
...
export const authHelpers = {
  ...
  async signInWithGoogle(redirectUrl = null, planId = null) {
    console.log('[Auth] Google OAuth with params:', { redirectUrl, planId });

    // Build query params to append to the callback URL
    const queryParams = {};
    if (redirectUrl) queryParams.redirect = redirectUrl;
    ...
    const { data, error } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        queryParams: queryParams, // Supabase will append these to the callback URL
      },
    });

    console.log('[Auth] OAuth initiated:', { data, error });
    return { data, error };
  },
